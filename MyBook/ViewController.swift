...
//        self.showLoginPopup()
    ...
    func loadBookData() -> [BookInfoWeb] {
        let book1 = BookInfoWeb(newURL:"http://www.yes24.com/24/goods/25863869?scode=029#contentsIntro", newTitle: "iOS 9 프로그래밍", newWriter: "닐 스미스", newDescription: "[핵심만 골라 배우는 iOS 9 프로그래밍]은 iOS 9 SDK와 Xcode 7, 그리고 스위프트 2 프로그래밍 언어를 이용하여 iOS 애플리케이션을 생성하는 데 필요한 기술을 가르치는 데 목적을 둔다. 독자들이 iOS 개발이 처음인지, 아니면 iOS 8 개발 경험이 있어서 iOS 9의 기능들과 최신 스위프트 프로그래밍 언어를 빠르게 습득하고자 하는지에 따라 이 책의 활용 방법이 크게 달라지겠지만, 이 책은 양쪽 모두를 대상으로 하는 iOS 참고서 역할을 하므로 안심하고 배울 수 있을 것이다.", newCover: UIImage(named: "1"))
        let book2 = BookInfoWeb(newURL:"http://www.yes24.com/24/goods/20477329?scode=029#contentsIntro", newTitle: "Swift로 하는 iOS 프로그래밍", newWriter: "매트뉴버그", newDescription: "『Swift로 하는 iOS 프로그래밍』는 스위프트(Swift) 언어를 이용해서 iOS 애플리케이션을 개발하려는 프로그래머, 오브젝티브C 문법에 익숙한 개발자, 그리고 iOS 인터페이스 구성과 작동 원리, 구현 철학을 이해하려는 UI 디자이너를 위한 iOS 앱 개발 입문서로 스위프트의 변수와 함수, 스코프(Scope)와 네임스페이스, 객체 타입과 인스턴스 등을 통해 객체지향형 콘셉트를 소개한다.", newCover: UIImage(named: "2"))
        let book3 = BookInfoWeb(newURL:"http://www.yes24.com/24/goods/26173805?scode=029#contentsIntro", newTitle: "구글의 미래", newWriter: "토마스 슐츠", newDescription: "미래를 상대로 한 구글의 도박이 시작되었다! 최근 구글 딥마인드가 개발한 인공지능 알파고가 전세계를 충격에 빠뜨렸다. 먼 미래의 이야기라 여겼던 것들이 생각했던 것보다 더욱 가까이 다가와 있음을 체감하게 만들었다. 그로부터 시작된 과학기술이 바꿔놓을 미래에 대한 뜨거운 논쟁은 자연스럽게 구글의 행보에 대한 관심으로 이어졌다. 인공지능, 로봇기술, 생명과학이 주도하며 전에 없던 속도로 세상에 변화를 가져올 것이라는 제4차산업혁명 시대에 구글이 어떤 태세를 취하는지 이목이 집중되었다.", newCover: UIImage(named: "3"))
